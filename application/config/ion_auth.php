...
$config['sender_email']               = $CI->settings->sender_email; 	// Sender Email, info@example.com
$config['sender_name']                = $CI->settings->sender_name; 	// Sender Name, example

$config['default_group']              = $CI->settings->i_default_group; // Default group, use name
$config['admin_group']                = $CI->settings->i_admin_group;   // Default administrators group, use name
...
$config['min_password_length']        = $CI->settings->i_min_password; 	// Minimum Required Length of Password
$config['max_password_length']        = $CI->settings->i_max_password;  // Maximum Allowed Length of Password
$config['email_activation']           = $CI->settings->i_email_activation ? TRUE : FALSE; // Email Activation for registration
$config['manual_activation']          = TRUE;               // Manual Activation for registration
$config['remember_users']             = TRUE;                // Allow users to be remembered and enable auto-login
$config['user_expire']                = $CI->settings->i_email_activation ? $CI->settings->i_email_activation : 0; // How long to remember the user (seconds). Set to zero for no expiration
$config['user_extend_on_login']       = FALSE;               // Extend the users cookies every time they auto-login
$config['track_login_attempts']       = TRUE;                // Track the number of failed login attempts for each user or ip.
$config['track_login_ip_address']     = TRUE;                // Track login attempts by IP Address, if FALSE will track based on identity. (Default: TRUE)
$config['maximum_login_attempts']     = $CI->settings->i_max_login_attempts; // The maximum number of failed login attempts.
$config['lockout_time']               = $CI->settings->i_lockout_time;      /* The number of seconds to lockout an account due to exceeded attempts. You should not use a value below 60 (1 minute) */
$config['forgot_password_expiration'] = $CI->settings->i_forgot_password_expiration ? $CI->settings->i_forgot_password_expiration : 0;                   // The number of milliseconds after which a forgot password request will expire. If set to 0, forgot password requests will not expire.
