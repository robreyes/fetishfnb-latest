...
class Blogs extends Admin_Controller {
    ...
    **/
    ...
    function index()
    {
        if(!$this->acl->get_method_permission($_SESSION['groups_id'], 'blogs', 'p_view'))
        {
            $this->session->set_flashdata('error', sprintf(lang('manage_acl_permission_no'), lang('menu_blog').' '.lang('manage_acl_view')));
