...
class Myevents extends Private_Controller {
    ...
    public function edit_myevent($id = NULL){
      ...
      if($id)
      {
          ...
          // For timeslots
          $_POST['timeslot']     = $result->custom_timeslot;
      }
    ...
    if(!empty($result->custom_timeslot)){
      $_POST['timeslot']= $result->custom_timeslot;

      $get_timeslots            = $this->event_model->get_timeslots($id);
      $result_timeslots_start   = json_decode($get_timeslots->slot_time_start);
      $result_timeslots_end     = json_decode($get_timeslots->slot_time_end);
      $capacity                 = json_decode($get_timeslots->slot_capacity);

      $timeslot_form_data = array();

      foreach ($result_timeslots_start as $index => $timeslot) {
        $timeslots = (object)(array());

        $timeslots->start_time         = date("g:i A", strtotime($timeslot));
        $timeslots->start_time_1       = explode(':', $timeslots->start_time)[0];
        $timeslots->start_time_2       = explode(':', $timeslots->start_time)[1];
        $timeslots->start_time_3       = explode(' ', $timeslots->start_time_2)[1];

        $timeslots->end_time           = date("g:i A", strtotime($result_timeslots_end[$index]));
        $timeslots->end_time_1         = explode(':', $timeslots->end_time)[0];
        $timeslots->end_time_2         = explode(':', $timeslots->end_time)[1];
        $timeslots->end_time_3         = explode(' ', $timeslots->end_time_2)[1];

        $timeslots->capacity           = $capacity[$index];

        $timeslot_data[] = $timeslots;
      }

      foreach ($timeslot_data as $index => $form_data) {
        $timeslot_form_data = array();

        $timeslot_form_data['timeslot_start_time_1']= array(
            'name'          => 'timeslot_start_time_hr[]',
            'id'            => 'timeslot_start_time_hr_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                '01'  => '01',
                                '02'  => '02',
          ...
          'selected'      => $this->form_validation->set_value('timeslot_start_time_1', !empty($form_data->start_time_1) ? $form_data->start_time_1 : ''),
        );

        $timeslot_form_data['timeslot_start_time_2']= array(
            'name'          => 'timeslot_start_time_min[]',
            'id'            => 'timeslot_start_time_min_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                '00' => '00',
                                '30' => '30',
                            ),
            'selected'      => $this->form_validation->set_value('timeslot_start_time_2', !empty($form_data->start_time_2) ? $form_data->start_time_2 : ''),
        );

        $timeslot_form_data['timeslot_start_time_3']= array(
            'name'          => 'timeslot_start_time_d[]',
            'id'            => 'timeslot_start_time_d_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                'AM' => 'AM',
                                'PM' => 'PM',
                            ),
            'selected'      => $this->form_validation->set_value('timeslot_start_time_3', !empty($form_data->start_time_3) ? $form_data->start_time_3 : ''),
        );


        $timeslot_form_data['timeslot_end_time_1']= array(
            'name'          => 'timeslot_end_time_hr[]',
            'id'            => 'timeslot_end_time_hr_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                '01'  => '01',
                                '02'  => '02',
          ...
          'selected'      => $this->form_validation->set_value('timeslot_end_time_1', !empty($form_data->end_time_1) ? $form_data->end_time_1 : ''),
        );

        $timeslot_form_data['timeslot_end_time_2']= array(
            'name'          => 'timeslot_end_time_min[]',
            'id'            => 'timeslot_end_time_min_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                '00' => '00',
                                '30' => '30',
                            ),
            'selected'      => $this->form_validation->set_value('timeslot_end_time_2', !empty($form_data->end_time_2) ? $form_data->end_time_2 : ''),
        );

        $timeslot_form_data['timeslot_end_time_3']= array(
            'name'          => 'timeslot_end_time_d[]',
            'id'            => 'timeslot_end_time_d_'.bcadd($index, 1),
            'class'         => 'form-control show-tick',
            'options'       => array(
                                'AM' => 'AM',
                                'PM' => 'PM',
                            ),
            'selected'      => $this->form_validation->set_value('timeslot_end_time_3', !empty($form_data->end_time_3) ? $form_data->end_time_3 : ''),
        );

        $timeslot_form_data['timeslot_capacity']= array(
            'name'          => 'timeslot_capacity[]',
            'id'            => 'timeslot_slots_'.bcadd($index, 1),
            'type'          => 'number',
            'min'           => '1',
            'placeholder'   => 'Slots',
            'class'         => 'form-control',
            'value'         => $this->form_validation->set_value('timeslot_capacity', !empty($form_data->capacity) ? $form_data->capacity : ''),
        );


        $the_form_data[] = $timeslot_form_data;
      }

      $data['timeslots'] = $the_form_data;
    }
      /* Load Template */
  ...
  */
  public function save()
  {
      ...
      if(isset($_POST['timeslot']))
          ...
          {
              $this->form_validation
              ->set_rules('timeslots_start_time[]', lang('event_slot_time'), 'trim|required')
              ->set_rules('timeslots_end_time[]', lang('event_slot_time'), 'trim|required')
              ->set_rules('timeslots_slot[]', lang('events_timeslot'), 'trim|required|is_natural_no_zero');
          }
      ...
      if(isset($_POST['timeslot']))
      {
        ...
        $timeslot_data = array();
        $timeslot_data['event_id']      = $id;

        $timeslotsstart  = $this->input->post('timeslots_start_time[]');
        $timeslotsend    = $this->input->post('timeslots_end_time[]');
        $capacity        = $this->input->post('timeslots_slot[]');

        $timeslotsstart_array = array();
        $timeslotsend_array = array();

        foreach($timeslotsstart as $index => $timeslotstart){
          $timeslotsstart_array[] = date("H:i", strtotime($timeslotstart));
          $timeslotsend_array[]   = date("H:i", strtotime($timeslotsend[$index]));
        }

        $timeslot_data['slot_time_start']   = json_encode($timeslotsstart_array);
        $timeslot_data['slot_time_end']     = json_encode($timeslotsend_array);
        $timeslot_data['slot_capacity']     = json_encode($capacity);

        $this->event_model->save_timeslots($timeslot_data, $id);
      }
