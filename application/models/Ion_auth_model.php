...
class Ion_auth_model extends CI_Model
{
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where("$this->tables['users'].username", $username)
										->group_by("$this->tables['users'].id")
										->order_by("$this->tables['users'].id", "ASC")
										->limit(1)
	...
	public function email_check($email = '')
	{
		...
		return $this->db->where("$this->tables['users'].email", $email)
										->group_by("$this->tables['users'].id")
										->order_by("$this->tables['users'].id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['users']) > 0;
	...
	**/
	 ...
	 **/
	public function users_in_group($group_id=FALSE)
	{
		$this->trigger_events('users_in_group');

		// if no id then return FALSE
		if(!$group_id)
			return FALSE;

		return $this->db->select($this->tables['users_groups'].'.user_id')
		                ->where($this->tables['users_groups'].'.group_id', $group_id)
		                ->get($this->tables['users_groups']);
	}
